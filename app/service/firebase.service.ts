import { DocumentData, QuerySnapshot, addDoc, collection, deleteDoc, doc, getDocs, onSnapshot, query, setDoc, where } from "@firebase/firestore"
...
export function FirebaseService() {
    ...
    const getData = async (collectionName: string, userId?: string): Promise<TODO[]> => {
        loading.enableLoading();
        const baseCollectionRef = collection(DB, collectionName);
        let finalRef;

        if (userId) {
            const filteredQuery = query(baseCollectionRef, where('userId', '==', userId));
        ...
        try {
            const querySnapshot: QuerySnapshot<DocumentData> = await getDocs(finalRef);
    ...
    const putData = async (collectionName: string, id: string, data: TODO): Promise<void> => {
        loading.enableLoading();
    ...
    const subscribeData = (collectionName: string, callback: (data: TODO[]) => void, userId?: string) => {
        const baseCollectionRef = collection(DB, collectionName);
        let finalRef;
